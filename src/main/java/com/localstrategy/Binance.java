            ...
            switch (orderAction) {
                case CREATE_ORDER -> {
                    ...
                    if (order.isAutomaticBorrow() && order.getType().equals(OrderType.LIMIT)) {
                        ...
                        if(isLong){
                            order.setBorrowedAmount(order.getSize() * fillPrice / tierManager.getCurrentLeverage());
                        }
                ...
                case CANCEL_ORDER -> {
                    ...
                    if (order.isAutoRepayAtCancel() && order.getMarginBuyBorrowAmount() != 0) {
            ...
            if(order.getStatus().equals(OrderStatus.NEW)){ //FIXME: Status field is mutable by the client. We're not editing it too much but still not good practice
                ...
                if(isMarketOrder ||
                    ...
                    if(isMarketOrder && order.isAutomaticBorrow()){
                        ...
                        //FIXME: Binance only checks for asset we must borrow
